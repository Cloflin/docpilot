 ...
 *
 * The batcher itself is `src/eval/prefetch.ts` now: `run.ts`, `tune.ts` and
 * `bench emit` embed the same way and all bought one at a time, and none of
 * them could reach a function that closed over this file's three `EMBED_*`
 * constants. What is left here is the closure — the target, the counter and the
 * progress line — and the map the loop below reads.
 */
...
async function prefetch(texts, index) {
  const { vectors, requests } = await prefetchEmbeddings(
    ...
    { onTick: (done, total) => tick(`embedded ${done}/${total} probe texts…`) },
  )
  EMBED_REQUESTS.count += requests
  for (const [t, v] of vectors) PREFETCHED.set(t, v)
  if (vectors.size) {
    tock(`embedded ${vectors.size} probe texts in ${requests} request(s), ${BATCH} at a time`)
  }
}
