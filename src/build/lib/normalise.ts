...
/**
 * MDX statements — `import`, `export`, and the whole-line JSX comment.
 *
 * MDX is markdown plus ES module syntax, and the module half is not prose. Left
 * in, `import Tabs from '@theme/Tabs'` becomes a sentence of the corpus: it is
 * indexed, it is retrievable, and on a Docusaurus site it is retrievable from
 * every page, because every page opens with the same four lines. A chunk that
 * says nothing and appears everywhere is the worst kind of noise a BM25 channel
 * can carry.
 *
 * THE COMPONENTS THEMSELVES NEED NOTHING HERE. `<Tabs>` is a tag, `stripHtml`
 * already takes tags and keeps their text, and its pattern is case-insensitive —
 * so a capitalised JSX element loses its brackets and keeps its children exactly
 * as `<em>` does. What is genuinely unlike markdown is only the module syntax.
 *
 * TWO GUARDS AGAINST EATING PROSE, because this runs over every page, not only
 * over `.mdx`, and a false positive here deletes documentation:
 *
 *   · the START must look like a statement and not like a sentence. `import` has
 *     to be followed by a quoted specifier, a `from '…'`, or the `{` of a
 *     multi-line list — so "import the file's contents" is prose despite its
 *     apostrophe, and "export the data as CSV" is prose because `export` must be
 *     followed by one of the declaration keywords;
 *   · the END is bracket balance, not a guess. `export const toc = [` runs until
 *     its `]` however many lines that takes, which is the only rule that does not
 *     stop in the middle of a multi-line statement and resume mid-expression.
 *
 * Fenced lines are untouched, like every other pass in this file: a page
 * DOCUMENTING an MDX import writes one in a sample, and deleting a sample's
 * first line is content loss with nothing on screen to show for it.
 */
export function stripMdx(src) {
  const START =
    /^import\s+(?:['"]|[^'"\n]*\bfrom\s+['"]|[^'"\n]*\{\s*$|[^'"\n]*,\s*$)|^export\s+(?:default|const|let|var|function|async|class|\*|\{)\b/
  ...
  eachLine(src, (line, fenced) => {
    ...
    if (!open && !START.test(line)) {
      // A whole-line JSX comment. Anything narrower — a comment sharing a line
      // with prose — is left alone rather than guessed at.
...
export function normaliseMarkdown(src) {
  ...
  // MDX first, and before applyLlmTags for the same reason applyLlmTags runs
  // before stripHtml: a module statement is not markup and must not reach a pass
  // that reads markup.
  let t = stripMdx(body)
