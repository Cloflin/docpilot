...
export function dedupe(
  ...
  { warn = (_message: string) => {} }: { warn?: (message: string) => void } = {},
) {
...
export async function proposeVocabulary({
  target,
  product,
  headings,
  languages,
...
export async function runVocabulary({ docPilot, argv = [], env = {}, out = null }) {
  ...
  const clean: Record<string, string[]> = dedupe(merged, { warn: (m) => say(`  ${m}`) })
