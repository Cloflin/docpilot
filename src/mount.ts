...
import type { DocPilotInstance, MountOptions } from '../types/mount.js'
 ...
 */
export function mountDocPilot(options: MountOptions = {}): DocPilotInstance {
  ...
  const mounted =
    trigger === 'none' || trigger == null
      ? []
      : (Array.isArray(trigger) ? trigger : [trigger]).filter((v) => (v as string) !== 'none')
