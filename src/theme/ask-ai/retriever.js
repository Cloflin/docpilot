 ...
 * and only `rag:calibrate` may set them (RAG-SPEC 7).
 */
const tune = (name, dflt) => {
  const raw = globalThis.process?.env?.[`ASK_AI_${name}`]
...
export function createRetrieval({ index, scope, guard, dev = false, onDebug = null }) {
  ...
  return {
    ...
    evaluate({ question, previousQuestion, queryVec, composedVec, mode = 'hybrid' }) {
      ...
      // rag:calibrate to record it per probe, and a boolean that only ever exists
    ...
    closest({ query, queryVec, outsideScope = false, limit = 3 }) {
      ...
      for (const c of scored) {
        ...
        out.push({ path: c.path, title: page?.title || c.title, tail: page?.tail || '' })
