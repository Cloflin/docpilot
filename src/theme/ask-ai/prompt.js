...
export const TOOLS = [
  {
    name: 'search_docs',
    description: 'Search the Stripo Plugin documentation. Returns ranked excerpts with ids.',
    parameters: {
 ...
 */
export const SHIPPED_CORE = `You answer questions about the Stripo Plugin using only the documentation excerpts you are given.
...
- If the question contains something that looks like a live Plugin ID, secret key, API key or token, open the answer by saying it should be treated as compromised and replaced from the Plugin details page in the Stripo account, do not quote the value, then answer the question as usual with its citations.
 ...
 *
 * `askAI.prompt` in docs/.vitepress/ask-ai.config.mjs may replace it outright
 ...
 */
export function coreText({override = null, extend = ''} = {}) {
  const base = typeof override === 'string' && override.trim() ? override.trim() : SHIPPED_CORE
...
export function detectLanguage(text) {
  ...
  // Stripo Plugin" is a Chinese question that happens to contain a product name,
...
export function clampAddendum(s) {
  return Array.from(
    String(s || '')
      .normalize('NFKC')
      .replace(/\p{Cf}/gu, ''),
...
export const promptHash = (prompt) =>
  fnv1a32(coreText(prompt) + TOOLS_DOC + OBS_DOC + OBS_NOTE + FINAL_NOTE)
 ...
 */
export function systemText({ scope, fallback = false, promptListLimit = 12, prompt } = {}) {
  const blocks = [coreText(prompt)]
...
export function buildMessages({
  ...
  const messages = [
    { role: 'system', content: systemText({ scope, fallback, promptListLimit, prompt }) },
  ]
...
export function promptDocument({
  ...
  return [
    ...
    { heading: 'Instructions', body: coreText(prompt) },
    {
      heading: fallback ? 'Tools (sent as text)' : 'Tools (delivered as tool definitions)',
      body: TOOLS_DOC,
    },
    { heading: 'Scope', body: sd || 'All documentation pages are searched.' },
    {
      heading: 'Your instruction',
      body: instruction || 'None. Anything you add here is sent as a separate message, after the instructions above and before your question.',
      yours: true,
    },
  ]
}
