...
const PATTERNS = [
  ...
  // The Stripo Plugin secret key, and every other hex digest used as one. Hex
...
const COPY = {
  English: {
    ...
    body: 'This one was not sent anywhere — the question stopped in the browser and the value is not kept in the thread. If it has already been pasted somewhere else, replace it on the Plugin details page in your Stripo account.',
    action: 'Answer the question without the key',
  },
  Russian: {
    lead: 'Не вставляйте сюда ключи и токены.',
    body: 'Этот ключ никуда не отправлен — вопрос остановился в браузере, значение не сохранено в переписке. Если вы уже вставляли его где-то ещё, замените его на странице Plugin details в аккаунте Stripo.',
    action: 'Ответить на вопрос без ключа',
  },
  Ukrainian: {
    lead: 'Не вставляйте сюди ключі та токени.',
    body: 'Цей ключ нікуди не надіслано — запитання зупинилося в браузері, значення не збережено в переписці. Якщо ви вже вставляли його десь інде, замініть його на сторінці Plugin details в акаунті Stripo.',
    action: 'Відповісти на запитання без ключа',
  },
  Spanish: {
    lead: 'No pegue aquí claves ni tokens.',
    body: 'Esta clave no se ha enviado a ninguna parte: la pregunta se detuvo en el navegador y el valor no se guarda en la conversación. Si ya la ha pegado en otro sitio, sustitúyala en la página Plugin details de su cuenta de Stripo.',
    action: 'Responder a la pregunta sin la clave',
  },
  Portuguese: {
    lead: 'Não cole chaves nem tokens aqui.',
    body: 'Esta chave não foi enviada para lugar nenhum — a pergunta parou no navegador e o valor não fica guardado na conversa. Se você já a colou em outro lugar, substitua-a na página Plugin details da sua conta Stripo.',
    action: 'Responder à pergunta sem a chave',
  },
  French: {
    lead: 'Ne collez pas de clés ni de jetons ici.',
    body: "Cette clé n'a été envoyée nulle part : la question s'est arrêtée dans le navigateur et la valeur n'est pas conservée dans la conversation. Si vous l'avez déjà collée ailleurs, remplacez-la depuis la page Plugin details de votre compte Stripo.",
    action: 'Répondre à la question sans la clé',
  },
  German: {
    lead: 'Fügen Sie hier keine Schlüssel oder Tokens ein.',
    body: 'Dieser Schlüssel wurde nirgendwohin gesendet — die Frage ist im Browser geblieben und der Wert wird nicht im Verlauf gespeichert. Falls Sie ihn bereits anderswo eingefügt haben, ersetzen Sie ihn auf der Seite Plugin details in Ihrem Stripo-Konto.',
    action: 'Die Frage ohne den Schlüssel beantworten',
  },
  Italian: {
    lead: 'Non incollare qui chiavi o token.',
    body: "Questa chiave non è stata inviata da nessuna parte: la domanda si è fermata nel browser e il valore non viene conservato nella conversazione. Se è già stata incollata altrove, sostituirla nella pagina Plugin details dell'account Stripo.",
    action: 'Rispondere alla domanda senza la chiave',
  },
  Polish: {
    lead: 'Nie wklejaj tutaj kluczy ani tokenów.',
    body: 'Ten klucz nie został nigdzie wysłany — pytanie zatrzymało się w przeglądarce, a wartość nie jest zapisywana w rozmowie. Jeśli był już wklejony gdzie indziej, zastąp go na stronie Plugin details w koncie Stripo.',
    action: 'Odpowiedz na pytanie bez klucza',
  },
  Turkish: {
    lead: 'Buraya anahtar veya token yapıştırmayın.',
    body: 'Bu anahtar hiçbir yere gönderilmedi — soru tarayıcıda kaldı ve değer sohbette saklanmıyor. Daha önce başka bir yere yapıştırdıysanız, Stripo hesabınızdaki Plugin details sayfasından değiştirin.',
    action: 'Soruyu anahtar olmadan yanıtla',
  },
  Japanese: {
    lead: 'ここにキーやトークンを貼り付けないでください。',
    body: 'このキーはどこにも送信されていません。質問はブラウザーで止まり、値は履歴に保存されていません。すでに他の場所に貼り付けている場合は、Stripo アカウントの Plugin details ページで置き換えてください。',
    action: 'キーなしで質問に回答する',
  },
  Korean: {
    lead: '여기에 키나 토큰을 붙여넣지 마세요.',
    body: '이 키는 어디에도 전송되지 않았습니다. 질문은 브라우저에서 멈췄고 값은 대화에 저장되지 않습니다. 이미 다른 곳에 붙여넣었다면 Stripo 계정의 Plugin details 페이지에서 교체하세요.',
    action: '키 없이 질문에 답변하기',
  },
  Chinese: {
    lead: '请不要在此处粘贴密钥或令牌。',
    body: '该密钥没有发送到任何地方——问题止步于浏览器，其值不会保存在会话记录中。如果你已经把它粘贴到别处，请在 Stripo 账户的 Plugin details 页面替换它。',
    action: '不带密钥回答这个问题',
  },
  Arabic: {
    lead: 'لا تلصق المفاتيح أو الرموز هنا.',
    body: 'لم يُرسل هذا المفتاح إلى أي مكان — توقّف السؤال داخل المتصفّح ولم تُحفظ القيمة في المحادثة. إذا سبق أن لصقته في مكان آخر، فاستبدله من صفحة Plugin details في حساب Stripo.',
    action: 'الإجابة عن السؤال بدون المفتاح',
  },
  Hebrew: {
    lead: 'אין להדביק כאן מפתחות או טוקנים.',
    body: 'המפתח הזה לא נשלח לשום מקום — השאלה נעצרה בדפדפן והערך אינו נשמר בשיחה. אם כבר הדבקתם אותו במקום אחר, החליפו אותו בעמוד Plugin details בחשבון Stripo.',
    action: 'לענות על השאלה בלי המפתח',
  },
  Hindi: {
    lead: 'यहाँ कुंजियाँ या टोकन न चिपकाएँ।',
    body: 'यह कुंजी कहीं नहीं भेजी गई — प्रश्न ब्राउज़र में ही रुक गया और मान बातचीत में सहेजा नहीं जाता। अगर आपने इसे पहले कहीं और चिपकाया है, तो अपने Stripo खाते के Plugin details पृष्ठ पर इसे बदल दें।',
    action: 'कुंजी के बिना प्रश्न का उत्तर दें',
  },
  Greek: {
    lead: 'Μην επικολλάτε κλειδιά ή διακριτικά εδώ.',
    body: 'Αυτό το κλειδί δεν στάλθηκε πουθενά — η ερώτηση σταμάτησε στο πρόγραμμα περιήγησης και η τιμή δεν αποθηκεύεται στη συνομιλία. Αν το έχετε ήδη επικολλήσει κάπου αλλού, αντικαταστήστε το από τη σελίδα Plugin details στον λογαριασμό σας Stripo.',
    action: 'Απάντηση στην ερώτηση χωρίς το κλειδί',
  },
  Thai: {
    lead: 'อย่าวางคีย์หรือโทเคนที่นี่',
    body: 'คีย์นี้ไม่ได้ถูกส่งไปที่ใด — คำถามหยุดอยู่ในเบราว์เซอร์ และค่าไม่ได้ถูกเก็บไว้ในบทสนทนา หากคุณเคยวางคีย์นี้ไว้ที่อื่นแล้ว ให้เปลี่ยนคีย์ในหน้า Plugin details ในบัญชี Stripo ของคุณ',
...
export function credentialCopy(language) {
