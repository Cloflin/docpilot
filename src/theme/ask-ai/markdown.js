...
import { GLYPHS, ICON_ATTRS } from './glyphs.js'
 ...
 * button on.
 *
 * The container class is `ask-ai__code` and must never contain the substring
 ...
 */
const COPY_BUTTON =
  '<button type="button" class="ask-ai__code-copy" data-copy-code aria-label="Copy code">' +
  `<svg ${ICON_ATTRS}>` +
  `<path class="ask-ai__glyph-rest" d="${GLYPHS.copy}"/>` +
  `<path class="ask-ai__glyph-done" d="${GLYPHS.check}"/>` +
  '</svg></button>'
...
md.renderer.rules.fence = (tokens, idx) => {
  ...
  return `<div class="ask-ai__code"${lang ? ` data-lang="${lang}"` : ''}>${COPY_BUTTON}${body}</div>\n`
}
