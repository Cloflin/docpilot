...
/**
 * How many neighbouring sections one turn may pull in.
 *
 * `expand_section` exists to cross a boundary, not to read a page a chunk at a
 * time. Two covers the case it was built for — the answer starts at the end of
 * one section and finishes at the start of the next, in either direction — and
 * stops the walk that a model finding expansion cheaper than search would
 * otherwise take.
 */
 ...
 */
/**
 * Everything a turn is given. Every reporter is optional because the eval
 * runner wires none of them and the panel wires all of them — an unannotated
 * destructured parameter would have made each one required.
 */
...
export async function runTurn(options: RunTurnOptions) {
  ...
  /**
   * Expansions taken this turn. Never refunded — see the cap below.
   */
  let expansions = 0
  ...
  async function execute(name, args) {
    ...
    if (name === 'expand_section') {
      const res = retrieval.expand(String(args.id || ''), args.direction === 'prev' ? 'prev' : 'next')
      if (!res.ok) {
        // `no-neighbour` is its own sentence and the other two are not. "There is
        // nothing after this section" is a fact about a page the model has already
        // been shown; "unknown id" has to stay indistinguishable from "out of
        // scope" for the reason `fetch_section` states above.
        rejectedFetches++
        return {
          error: res.reason === 'no-neighbour' ? 'no neighbouring section' : 'unknown id',
          charge: rejectedFetches === 1,
        }
      }
      // THE CAP IS ON THE TURN, NOT ON THE PAGE. Without one, a model that finds
      // walking cheaper than searching walks: `next` from the first chunk to the
      // last, one step per iteration, spending the whole budget on a page the
      // gate already ranked. Two is enough to cross a boundary in either
      // direction and not enough to read a page sideways.
      if (expansions >= MAX_EXPANSIONS) {
        return { error: 'no more expansions this turn', charge: true }
      }
      expansions++
      onPhase?.({ phase: 'reading', label: res.section.title })
      // Citable for the same reason a searched id is: it is text the host put in
      // front of the model this turn.
      emittedIds.add(res.section.id)
      spelled.add(res.section.id)
      return {
        observation: observation('expand_section', scopeLabel, [
          {
            id: res.section.id,
            title: res.section.title,
            path: res.section.path,
