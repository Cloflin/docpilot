...
/**
 * What every reader of a scope may assume. `label` is written one line after the
 * object is built rather than in it — `labelFor` needs the kind and the paths to
 * decide — so the literal is asserted into this shape rather than declared as
 * it, which keeps the emitted code identical to what it was.
 */
export interface Scope {
  kind: 'all' | 'page' | 'section' | 'custom'
  paths: string[]
  label: string
}
...
export function makeScope(paths, manifest) {
  ...
  const scope = { kind: unique.length === 1 ? 'page' : 'custom', paths: unique } as Scope
