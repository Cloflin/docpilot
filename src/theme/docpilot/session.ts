...
/**
 * The drawer's whole state — one reactive object, module-level, read by all four
 * components and written only from this file.
 *
 * Same two-block split as `Turn`: the fields below the marker are written after
 * something is learned, and `reactive({…})` could not infer them from a literal
 * that does not mention them. `config` is the RESOLVED theme config rather than
 * the author's settings — `themeDocPilot` emits it and `types/config.d.ts`
 * describes it as `DocPilotThemeConfig`, but the resolved `llm` block is a
 * superset of what that interface names, so this stays open until the
 * `ResolvedDocPilot` split lands.
 */
export interface SessionState {
  ...
  busy: boolean
  dockPanel: 'picker' | 'prompt' | 'history' | null
  instruction: string
  announce: string
  ...
  /** Learned rather than initialised — see the note above. */
...
const T = (path: string, vars?: Record<string, string | number>) => t(tree(), normaliseLocale(state.lang, tree()), path, vars)

/** A `reply.*` string: keyed by the typed language, with the page's as backup. */
const replyT = (locale: string, path: string, vars?: Record<string, string | number>) =>
...
export async function ensureIndex() {
  ...
  try {
    ...
    // The reason is deliberately NOT kept. It was, and nothing ever read it —
    // ui-specs/012 deletes it on the reasoning 009 used for `llm.think`: state
    // with no reader is indistinguishable from state somebody forgot to wire,
    // and an upstream message held in the store is a message one careless
    // template binding away from the reader. `degraded` is the whole fact the
    // panel acts on.
    state.degraded = true
  }
}
...
/**
 * The one write that happens MID-STREAM, and only because the page is leaving.
 * ui-specs/012.
 *
 * `saveCurrent` above is deliberately never called while a turn runs. This is
 * the exception the rule was missing: at `pagehide` the alternative to one write
 * is losing every token already streamed, on a request that has been paid for.
 *
 * IDEMPOTENT FOR ONE TURN, and that is not tidiness. `visibilitychange` is bound
 * beside `pagehide` because mobile Safari can discard a tab without firing the
 * latter — and it fires on every tab switch, app switch and screen lock. Without
 * the marker a reader who swaps apps while an answer streams rewrites the
 * archive on every swipe.
 *
 * The marker is (turn, length), not just the turn. A tab that is hidden, comes
 * BACK — bfcache restore is a live page, and the stream is still running — and
 * is hidden again has more text the second time, and that text is worth the
 * second write. What the marker stops is the case with nothing new in it, which
 * is the swipe.
 *
 * Module-level rather than a field on the turn: only one turn runs at a time,
 * because `submit` returns early on `state.busy`, so this is a single value —
 * and a field would have to be added to `makeTurn` AND to the restore shape
 * beside it for no gain.
 *
 * A turn with nothing in it yet writes nothing at all: `slimTurn` returns null
 * when there is neither an answer nor a refusal, which is what keeps a reader
 * who reloads two seconds after asking from finding an empty row.
 */
let savedOnUnload = null

export function saveIfRunning() {
  if (!state.busy) return
  if (!state.config.history.enabled || !state.config.history.saveOnUnload) return
  const turn = state.turns[state.turns.length - 1]
  if (!turn) return
  const written = (turn.answerText || '').length
  if (savedOnUnload?.turn === turn && savedOnUnload.written >= written) return
  savedOnUnload = { turn, written }
  saveCurrent()
}
...
/**
 * One turn, live — the record the panel renders and `slimTurn` trims for the
 * archive.
 *
 * Split in two on purpose. The first block is what `makeTurn` writes, so those
 * fields are always there; everything under it is written LATER, on the branch
 * that learned it — a gate verdict, a model name, an error, a rate-limit
 * snapshot — and is optional for exactly that reason. An unannotated
 * `reactive({…})` inferred only the first block, which is why every one of the
 * second block's fields was an error the moment this file became TypeScript.
 */
export interface Turn {
  ...
  wouldWiden: boolean
  closest: any[]
  closestAreOutside: boolean
  hybrid: boolean
  ladder: any
  refusal: any
  thought: string
  thoughtOpen: boolean
  thoughtChoice: boolean | null
  ...
  /** Written on the branch that learns it — see the note above. */
  gate?: any
  model?: string
  error?: string
  cited?: any[]
