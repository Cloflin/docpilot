 ...
 * @typedef {Error & {status?: number, retryAfterMs?: number, rateLimit?: RateLimit}} TransportError
 */
 ...
 */
function reportHeaders(onHeaders, res, kind) {
  if (!onHeaders) return
  try {
...
/** Test seam: forget every sticky choice and every cooldown. */
...
export async function chat({
  ...
  const pool = poolFor(provider, baseURL)
  const candidates = orderCandidates([model, ...(models || [])], { ...pool, primary: model || null })
   ...
   */
  const reachable = (last) => rotating && !last && spent + 1 < ceiling
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (wrong && worthRotating && rotating && !last && !emitted && affordable()) {
