...
const UI_EN = {
  ...
  status: {
    ...
    /**
     * The two escalation steps — ui-specs/012. Neither names a cause and
     * neither promises a next action, because a chain with one member never
     * rotates and a line this panel prints has to be true in every
     * configuration. The second is safe to state as a fact because the
     * escalation only runs while nothing has painted: `status.js` makes it the
     * condition rather than an observation.
     */
    stillWorking: 'Still working',
    takingAWhile: 'Still working — the answer has not started yet',
  },
  turn: {
    stopped: 'Stopped.',
 ...
 */
export function validateI18n(i18n: unknown, warn = console.warn) {
  const out: {
  ...
  const given = i18n as { translations?: unknown; locales?: Record<string, { translations?: unknown }> }
  copy(given.translations, out.translations, 'translations.')
  for (const [locale, block] of Object.entries(given.locales || {})) {
