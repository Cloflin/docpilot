...
import { setVocabulary, setTokenizer } from './text.js'
...
export function assembleIndex({ manifest, shards, vectorBuffer, dfDoc }) {
   ...
   */
  // Before `setVocabulary`, because `vocabularyHash` reads both and the build
  // installs them in this order too. Absent on every index built before the key
  // existed, which reads as off — the behaviour those indexes were built with.
