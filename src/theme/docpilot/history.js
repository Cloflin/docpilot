...
export function slimTurn(turn, limits = LIMITS) {
  ...
  const out = { id: turn.id, question: turn.question, state, answerText, sources }
 ...
 * ReferenceError.
 */
export function createHistory({ local, session, now, limits } = {}) {
  ...
  return {
     ...
     */
    setLimits(next) {
      for (const [k, v] of Object.entries(next || {})) if (v > 0) caps[k] = v
    },
