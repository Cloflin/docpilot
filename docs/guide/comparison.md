...
- **Rebuilding the index draws on the same allowance** — but only for what
  changed. Vectors are cached by model and by chunk text, so editing one page
  costs one embedding request rather than one per 32 chunks of the corpus. A
  first build, or an embedder swap, still pays in full.
...
| **Index the reader downloads** | int8 vectors, one byte per dimension — 855 KB for this site's 570 chunks at 1536 dimensions | n/a — the index is theirs | n/a — the index is theirs | n/a — the index is theirs | n/a — the index is theirs | n/a, unless you ship an OSS bundle |
...
| **Reader-facing strings replaceable** | all 173 of them, one at a time, in 25 groups | vendor's locales |
...
a test in this repository. The index figures — 570 chunks, 1536 dimensions,
855 KB of int8 vectors, under 0.003 mean |Δcos| — describe the files this page
