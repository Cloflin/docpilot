...
The three questions this panel is opened for. They are also the three openers on
its empty state, answered in the config so a click costs nothing — see
[`suggestions.authored`](/reference/config#suggestions-authored). This page is
why a reader who phrases one of them differently still gets the same answer:
retrieval finds it here, at any wording and in any language.
