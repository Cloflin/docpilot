...
The antecedent reaches **one turn further back** when the previous turn was itself an ellipsis — when it only scored once its own antecedent was prepended, which is a measurement the gate has already taken rather than a guess about the shape of a question — and the chain stops at two. The three bounds hold unchanged over the longer antecedent: the score is a maximum, so it can still only reduce refusals; admissibility is still measured against the reader's **own** words, the raw tail against the retrieved evidence; and the lexical weight is still below the threshold by assertion, so borrowed vocabulary clears nothing on its own.
